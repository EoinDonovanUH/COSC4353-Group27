...
const { ClientInformation, clients } = require("../models/ClientInformation");
...
const getAllClients = asyncHandler(async (req, res) => {
  ...
  const clientWithUser = await Promise.all(
    clients.map(async (client) => {
      const user = await UserCredentials.findById(client.user_credentials)
        .lean()
        .exec();
